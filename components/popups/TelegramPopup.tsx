...
function TelegramPopup() {
  const [visible, setVisible] = usePopupContext('telegram')
  ...
  return (
    <BasicPopup isOpen={visible} onClose={onClose}>
      <Box as="form" className="form" onSubmit={onSubmit}>
        <ModalHeader>
          <h2 className="main-title center">Telegram帐号修改</h2>
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody mt="4">
          <FormControl className="form-group">
            <FormLabel>Telegram帐号</FormLabel>
            <Input
              className="form-input"
              placeholder="请输入Telegram帐号"
              name="telegram_id"
